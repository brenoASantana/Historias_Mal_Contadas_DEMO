...
// Draw a cone light
void DrawConeLight(Vector2 center, float radius, float angle, float spread, Color color)
{
    int segments = 32;
    float step = spread / segments;
    float startAngle = angle - spread / 2.0f;
...
int main(void)
{
    ...
    // Setup initial light
    SetupLight(0, screenWidth / 2, screenHeight / 2, 300);
    ...
    float coneRadius = 150.0f;
    float coneSpread = PI / 4; // 45 degrees
    ...
    while (!WindowShouldClose())    // Detect window close button or ESC key
    {
        // Update
        //----------------------------------------------------------------------------------
        // Control light position with arrow keys
        if (IsKeyDown(KEY_RIGHT)) lights[0].position.x += 5.0f;
        if (IsKeyDown(KEY_LEFT)) lights[0].position.x -= 5.0f;
        if (IsKeyDown(KEY_UP)) lights[0].position.y -= 5.0f;
        if (IsKeyDown(KEY_DOWN)) lights[0].position.y += 5.0f;

        // Update cone angle to point towards mouse
        Vector2 mousePosition = GetMousePosition();
        float coneAngle = atan2f(mousePosition.y - lights[0].position.y, mousePosition.x - lights[0].position.x);

        // Mark the light as dirty so it gets redrawn
        lights[0].dirty = true;
        ...
        BeginDrawing();
            ...
            // Draw the cone light
            DrawConeLight(lights[0].position, coneRadius, coneAngle, coneSpread, ColorAlpha(YELLOW, 0.8f));
            ...
            DrawText("Use arrow keys to move the light", 10, 10, 10, DARKGREEN);
